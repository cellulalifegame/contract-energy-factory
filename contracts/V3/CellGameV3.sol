...
import {LifeV3 as Life} from "./LifeV3.sol";
import "../Helps.sol";

interface IMinerStaking {
    function isMiner(address miner_address) external view returns (bool);
}

contract CellGameV3 is ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function setDevAddress(address devAddress_) external onlyOwner {
        require(devAddress_ != address(0), 'invalid address');
        emit ChangeDevAddress(_devAddress, devAddress_);
        _devAddress = devAddress_;
    }

    function setTreasuryAddress(address treasuryAddress_) external onlyOwner {
        require(treasuryAddress_ != address(0), 'invalid address');
        emit ChangeTreasuryAddress(_treasuryAddress, treasuryAddress_);
        _treasuryAddress = treasuryAddress_;
    }

    function setAdmin(address admin_) external onlyOwner {
        require(admin_ != address(0), 'invalid address');
        emit ChangeAdminAddress(_admin, admin_);
        _admin = admin_;
    }
    ...
    function setLifeCreationConfig(
        ...
        if (_lifeCreationConfig.payToken == address(0)) {
    ...
    function getCellRentPrice(
        ...
        return vrgdaPrice > cellFloorPrice ? vrgdaPrice : cellFloorPrice;
    }
    ...
    function getLifePrice(
        ...
        require(
            cells_.length >= 2 && cells_.length <= 16,
            "can only use 2-16 cells!"
        );
    ...
    function createLife(uint256[][] memory cellsPositions_) public payable returns(uint256 life_token_id) {
        require(pauseStatus == false, "pause");
        require(IMinerStaking(minerStakingAddress).isMiner(msg.sender), "invalid miner address");
        require(
            cellsPositions_.length >= 2 && cellsPositions_.length <= 16,
            "can only use 2-16 cells!"
        );
        ...
        for (uint256 i = 0; i < cellsPositions_.length; ++i) {
            ...
            if (tokenPaymentStatus) {
                _rentTokenFeeCollected[tokenId] += rentFee;
            } else {
        ...
        if (tokenPaymentStatus) {
            _totalRentTokenFee += totalRentFeeCollected;
            _devTokenFeeCollected += (remainFee * 14) / 19 ;
            _poolTokenFeeCollected += remainFee - (remainFee * 14) / 19;
        } else {
            _totalRentFee += totalRentFeeCollected;
        ...
        if (tokenPaymentStatus) {
            ...
            IERC20(_lifeCreationConfig.payToken).safeTransferFrom(address(msg.sender), address(this), cumulatedPrice);
        } else {
            require(msg.value >= cumulatedPrice, "Insufficient funds");
        }

        life_token_id = _life.createLife(
            msg.sender,
            cumulatedPrice,
            cellsPositions_,
            cellGenes,
        ...
        if (msg.value > cumulatedPrice) {
            (bool sent, ) = payable(msg.sender).call{ value: msg.value - cumulatedPrice }(""); // Returns false on failure
    ...
    bool public pauseStatus; // Control charging and mint
    bool public tokenPaymentStatus;
    uint256 public _totalRentTokenFee;
    uint256 public _devTokenFeeCollected;
    uint256 public _poolTokenFeeCollected;
    mapping(uint256 => uint256) public _rentTokenFeeCollected;

    event ChangePauseStatus(bool old_status, bool new_status);
    event ChangeTokenPaymentStatus(bool old_status, bool new_status);
    event WithdrawAvailableToken(address receiver, uint256 amount);
    event WithdrawRentTokenFee(address receiver, uint256[] token_ids, uint256 amount);
    event WithdrawDevTokenFee(address receiver, uint256 amount);
    event WithdrawPoolTokenFee(address receiver, uint256 amount);
    ...
    // withdraw available balance from the contract ($CELL)
    // The theoretical value of withdrawable is 0
    function withdrawAvailableToken(uint256 amount) public onlyOwner {
        require(_withdrawable, "withdraw paused");
        uint256 latest = IERC20(_lifeCreationConfig.payToken).balanceOf(address(this));
        uint256 withdrawable = latest - (_totalRentTokenFee + _devTokenFeeCollected + _poolTokenFeeCollected);
        require(amount <= withdrawable, "Insufficient balance");
        IERC20(_lifeCreationConfig.payToken).safeTransfer(msg.sender, amount);
        emit WithdrawAvailableToken(msg.sender, amount);
    }

    function getWithdrawRentTokenFee( uint256[] memory tokenIds ) public view returns (uint256) {
        uint256[] memory uniqueArray = removeDuplicates(tokenIds);
        uint256 rentFeeSum = 0;
        for (uint256 i = 0; i < uniqueArray.length; ++i) {
            rentFeeSum += _rentTokenFeeCollected[uniqueArray[i]];
        }
        return rentFeeSum;
    }

    // withdraw rent fee from the contract ($CELL)
    function withdrawRentTokenFee(uint256[] memory tokenIds) public {
        require(_withdrawable, "withdraw paused");
        uint256[] memory uniqueArray = removeDuplicates(tokenIds);
        uint256 rentFeeSum = 0;
        for (uint256 i = 0; i < uniqueArray.length; ++i) {
            ...
            uint256 rentFee = _rentTokenFeeCollected[uniqueArray[i]];
            rentFeeSum += rentFee;
            _rentTokenFeeCollected[uniqueArray[i]] = 0;
            _totalRentTokenFee -= rentFee;
        }
        require(rentFeeSum > 0, "no rent fee");
        IERC20(_lifeCreationConfig.payToken).safeTransfer(msg.sender, rentFeeSum);
        emit WithdrawRentTokenFee(msg.sender, tokenIds, rentFeeSum);
    }

    // withdraw dev fee from the contract ($CELL)
    function withdrawDevTokenFee(address withdrawTo) public {
        require(_withdrawable, "withdraw paused");
        require(msg.sender == _devAddress, "must be dev");
        uint256 devFee = _devTokenFeeCollected;
        require(devFee > 0, "no dev fee");
        _devTokenFeeCollected = 0;
        IERC20(_lifeCreationConfig.payToken).safeTransfer(withdrawTo, devFee);
        emit WithdrawDevTokenFee(withdrawTo, devFee);
    }

    // withdraw pool fee from the contract ($CELL)
    function withdrawPoolTokenFee(address withdrawTo) public {
        require(_withdrawable, "withdraw paused");
        require(msg.sender == _treasuryAddress, "must be treasury");
        uint256 poolFee = _poolTokenFeeCollected;
        require(poolFee > 0, "no pool fee");
        _poolTokenFeeCollected = 0;
        IERC20(_lifeCreationConfig.payToken).safeTransfer(withdrawTo, poolFee);
        emit WithdrawPoolTokenFee(withdrawTo, poolFee);
    }

    // batch
    function batchCreateLife(uint256[][][] calldata genes, uint256[] calldata quantitys) public payable returns(uint256[] memory token_ids) {
        require(pauseStatus == false, "pause");
        require(IMinerStaking(minerStakingAddress).isMiner(msg.sender), "invalid miner address");
        require(genes.length == quantitys.length, 'params length error');
        
        // token_ids
        uint256 quantityTotal;
        for (uint256 i = 0; i < quantitys.length; ++i) {
            quantityTotal += quantitys[i];
        }
        token_ids = new uint256[](quantityTotal);
        uint256 ids_index;

        uint256 cumulatedPrice = 0; // Total value of all life
        uint256 totalRentFeeCollected = 0;

        // handler each gene
        for (uint256 i = 0; i < genes.length; ++i) {
            uint256 quantity = quantitys[i];
            uint256[][] calldata cellsPositions_ = genes[i];
            require( cellsPositions_.length >= 2 && cellsPositions_.length <= 16, "can only use 2-16 cells!");

            // data for cellGenes and livingCellTotals
            uint256[] memory cellGenes = new uint256[](cellsPositions_.length);
            uint32[] memory livingCellTotals = new uint32[](cellsPositions_.length);
            for (uint256 j = 0; j < cellsPositions_.length; ++j) {
                CellGene storage cellGene = _cellPool[cellsPositions_[j][0]];
                require(cellGene.bornTime > 0, "cell not minted");
                cellGenes[j] = cellGene.bitmap.getBucket(0);
                livingCellTotals[j] = cellGene.livingCellTotal;
            }

            // mint `quantity` life using single gene
            for (uint256 index = 0; index < quantity; ++index) {
                uint256 lifeBornPrice;

                // got cells price
                for (uint256 j = 0; j < cellsPositions_.length; ++j) {
                    uint256 tokenId = cellsPositions_[j][0];
                    CellGene storage cellGene = _cellPool[tokenId]; 
                    uint256 absoluteTimeSinceStart = block.timestamp - cellGene.bornTime;
                    uint256 cellRentPrice = getCellRentPrice( cellGene.rentedCount, absoluteTimeSinceStart);
                    cellGene.rentedCount += 1;

                    lifeBornPrice += cellRentPrice;
                    cumulatedPrice += cellRentPrice;
                    uint256 rentFee = (cellRentPrice * 5) / 100;
                    if (tokenPaymentStatus) {
                        _rentTokenFeeCollected[tokenId] += rentFee;
                ...
                token_ids[ids_index] = _life.createLife(msg.sender, lifeBornPrice, cellsPositions_, cellGenes, livingCellTotals);
                ++ids_index;
                lifeBornPrice = 0;
            }
        }
    ...
    // ============ V3 ============
    uint256 public cellFloorPrice;
    address public minerStakingAddress;

    event ChangeFloorPrice(uint256 old_price, uint256 new_price);
    event ChangeMinerStakingAddress(address old_address, address new_address);
    // Event For V1
    event ChangeDevAddress(address old_address, address new_address);
    event ChangeTreasuryAddress(address old_address, address new_address);
    event ChangeAdminAddress(address old_address, address new_address);

    function changeFloorPrice(uint256 new_price) public onlyOwner {
        require(new_price > 0, "invalid price");
        emit ChangeFloorPrice(cellFloorPrice, new_price);
        cellFloorPrice = new_price;
    }

    function setMinerStakingAddress(address new_address) public onlyOwner {
        require(new_address != address(0), "invalid address");
        require(minerStakingAddress == address(0), "repeat config");
        emit ChangeMinerStakingAddress(minerStakingAddress, new_address);
        minerStakingAddress = new_address;
    }
}