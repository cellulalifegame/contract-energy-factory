...
library VRGDA {
    ...
    function getTargetSaleTimeLinear(
        int256 sold,
        int256 perTimeUnit
    ...
    /// @dev Given a number of tokens sold, return the target time that number of tokens should be sold by using a logistic schedule
    /// @param sold A number of tokens sold, scaled by 1e18, to get the corresponding target sale time for.
    /// @param soldBySwitch The number of tokens that must be sold for the switch to occur.
    /// @param switchTime The time soldBySwitch tokens were targeted to sell by, scaled by 1e18.
    /// @param timeScale The steepness of the logistic curve, scaled by 1e18.
    /// @param logisticLimit The maximum number of tokens of tokens to sell + 1
    /// @return The target time the tokens should be sold by, scaled by 1e18, where the time is
    /// relative, such that 0 means the tokens should be sold immediately when the VRGDA begins.
    function getTargetSaleTimeLogisticToLinear(
        int256 sold,
        int256 soldBySwitch,
        int256 switchTime,
        int256 logisticLimit,
